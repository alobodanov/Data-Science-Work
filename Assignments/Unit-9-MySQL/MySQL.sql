...
-- 3b. You realize that some of these actors have tremendously long middle names. Change the data type
-- of the `middle_name` column to `blobs`.
...
-- correct name after all! In a single query, if the first name of the actor is currently `HARPO`, change it
-- to `GROUCHO`. Otherwise, change the first name to `MUCHO GROUCHO`, as that is exactly what the actor will
